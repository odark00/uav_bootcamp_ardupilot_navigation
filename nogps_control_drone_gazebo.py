...
# --- Tunables -------------------------------------------------------------- #
TARGET_ALT = 5.0        # desired altitude above launch point (m)
...
FORWARD_PITCH_DEG = 8.0  # nose-down angle used to fly forward
...
# --- Flight phases --------------------------------------------------------- #
def takeoff_to(mav, ground_hpa):
    """Hold level attitude with climb thrust until inside ALT_ACCURACY."""
    ...
    while True:
        ...
        send_attitude(mav, 0.0, 0.0, 0.0, thrust)  # level
...
def fly_forward(mav, ground_hpa):
    """Pitch forward while regulating height inside +/- ALT_BAND."""
    print(f"[*] Forward flight for {FORWARD_TIME:.0f} s, "
    ...
    while time.time() < end:
        ...
        thrust = alt_thrust(alt, TARGET_ALT)
        send_attitude(mav, 0.0, cmd_pitch, 0.0, thrust)
...
def descend_and_disarm(mav, ground_hpa):
    ...
    while True:
        ...
        send_attitude(mav, 0.0, 0.0, 0.0, thrust)
...
def mission(mav):
    ...
    takeoff_to(mav, ground_hpa)
    fly_forward(mav, ground_hpa)
    descend_and_disarm(mav, ground_hpa)
